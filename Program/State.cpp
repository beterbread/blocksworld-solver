...
int State::heuristic(const State& other, std::string H) const {
    ...
    // }
    if (H == "H2") {
        // Build goal position lookup
        std::unordered_map<char, std::pair<int,int>> goalPos;
        for (int i = 0; i < other.stacks.size(); i++)
            for (int j = 0; j < other.stacks[i].size(); j++)
                goalPos[other.stacks[i][j]] = {i, j};

        int res = 0;
        for (int i = 0; i < stacks.size(); i++) {
            bool stackCorrupted = false; // once a block is wrong, everything above must move
            for (int j = 0; j < stacks[i].size(); j++) {
                char block = stacks[i][j];
                auto [gs, gd] = goalPos[block];

                if (!stackCorrupted && i == gs && j == gd) {
                    // Check the block below is also correct (can't be right if foundation is wrong)
                    continue;
                }

                // This block needs to move - costs at least 1 move
                res += 1;
                stackCorrupted = true; // everything above this also needs to move
            }
        }
        return res;
    }
